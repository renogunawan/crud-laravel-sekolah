...
<h1 style="color: white"><center> GURU</center></h1> 

&nbsp;&nbsp;<a href="http://127.0.0.1:8000/guru/create" class="btn btn-primary"><i class="fa fa-user-plus"></i> Add Data</a>
...
<hr>
 ...
 <td><img src="{{ asset('storage/guru/' . $guru->image) }}" alt="{{ $guru->nama }}" style="max-height: 100px;"/></td>
                <td>{{ $guru->mapel->mapel }}</td>
                
                <td>
                    <center>
                        <form id="delete-form-{{ $guru->id }}" action="{{ route('guru.destroy', $guru->id) }}" method="POST">
                            <a href="{{ route('guru.edit', $guru->id) }}" class="btn btn-primary">Edit</a>
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger" onclick="showAlert(event, {{ $guru->id }})">Delete</button>
                        </form>
                    </center>
