...
<h1 style="color: white"><center> MAPEL</center></h1> 

&nbsp;&nbsp;<a href="http://127.0.0.1:8000/mapel/create" class="btn btn-primary"><i class="fa fa-user-plus"></i> Add Data</a> 
    ...
    <thead class="bg-success text-white" style="font-weight: bold">
       <tr>
        ...
        @forelse ($mapels as $mapel)
            <tr>
                <td>{{ $mapel->kode_buku }}</td>
                <td>{{ $mapel->mapel }}</td>

                <td>
                    <center>
                        <form id="delete-form-{{ $mapel->id }}" action="{{ route('mapel.destroy', $mapel->id) }}" method="POST">
                            <a href="{{ route('mapel.edit', $mapel->id) }}" class="btn btn-primary">Edit</a>
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger" onclick="showAlert(event, {{ $mapel->id }})">Delete</button>
                        </form>
                    </center>
