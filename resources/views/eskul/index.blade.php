...
<h1 style="color: white"><center> EKSTRAKULIKULER</center></h1> 

&nbsp;&nbsp;<a href="http://127.0.0.1:8000/eskul/create" class="btn btn-primary"><i class="fa fa-user-plus"></i> Add Data</a>
<form action="/eskul" method="GET">
       ...
       <tr>
        ...
        @forelse ($eskuls as $eskul)
            <tr>
                ...
                <td>{{ $eskul->kelas }}</td>
                <td>{{ $eskul->no_hp }}</td>
                <td>{{ $eskul->ekstrakulikuler }}</td>

                <td>
                    <center>
                        <form id="delete-form-{{ $eskul->id }}" action="{{ route('eskul.destroy', $eskul->id) }}" method="POST">
                            <a href="{{ route('eskul.edit', $eskul->id) }}" class="btn btn-primary">Edit</a>
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger" onclick="showAlert(event, {{ $eskul->id }})">Delete</button>
                        </form>
                    </center>
                    ...
                    <script>
                        function showAlert(event, id) {
                            ...
                            swal({
                                ...
                                text: "Data eskul akan dihapus secara permanen!",
                                icon: "warning",
                                buttons: true,
                                dangerMode: true,
                            })
